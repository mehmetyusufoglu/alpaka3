...
set(ALPAKA_HAS_RCCL OFF CACHE INTERNAL "RCCL detected" FORCE)
...
if(alpaka_ENABLE_COLLECTIVES AND alpaka_ENABLE_RCCL)
    set(_alpaka_rccl_hint_roots)
    foreach(var ROCM_PATH ROCM_HOME)
        if(DEFINED ENV{${var}} AND NOT "$ENV{${var}}" STREQUAL "")
            list(APPEND _alpaka_rccl_hint_roots "$ENV{${var}}")
        endif()
    endforeach()
    if(DEFINED rocblas_DIR)
        get_filename_component(_alpaka_rccl_from_rocblas "${rocblas_DIR}/../../.." REALPATH)
        if(_alpaka_rccl_from_rocblas)
            list(APPEND _alpaka_rccl_hint_roots "${_alpaka_rccl_from_rocblas}")
        endif()
    endif()
    if(DEFINED MIOpen_DIR)
        get_filename_component(_alpaka_rccl_from_miopen "${MIOpen_DIR}/../../.." REALPATH)
        if(_alpaka_rccl_from_miopen)
            list(APPEND _alpaka_rccl_hint_roots "${_alpaka_rccl_from_miopen}")
        endif()
    elseif(DEFINED miopen_DIR)
        get_filename_component(_alpaka_rccl_from_miopen "${miopen_DIR}/../../.." REALPATH)
        if(_alpaka_rccl_from_miopen)
            list(APPEND _alpaka_rccl_hint_roots "${_alpaka_rccl_from_miopen}")
        endif()
    endif()
    if(_alpaka_rccl_hint_roots)
        list(REMOVE_DUPLICATES _alpaka_rccl_hint_roots)
        set(_alpaka_rccl_hint_args HINTS ${_alpaka_rccl_hint_roots})
    else()
        unset(_alpaka_rccl_hint_args)
    endif()

    find_path(
        ALPAKA_RCCL_INCLUDE_DIR
        NAMES rccl/rccl.h rccl.h ${_alpaka_rccl_hint_args}
        PATH_SUFFIXES include include/rccl
    )

    find_library(ALPAKA_RCCL_LIBRARY NAMES rccl ${_alpaka_rccl_hint_args} PATH_SUFFIXES lib lib64)

    if(ALPAKA_RCCL_LIBRARY AND EXISTS "${ALPAKA_RCCL_LIBRARY}")
        if(ALPAKA_RCCL_INCLUDE_DIR)
            set(ALPAKA_HAS_RCCL ON CACHE INTERNAL "RCCL detected" FORCE)
            message(STATUS "RCCL library detected: ${ALPAKA_RCCL_LIBRARY}")
            if(NOT alpaka_DISABLE_VENDOR_RPATH)
                get_filename_component(_alpaka_rccl_dir "${ALPAKA_RCCL_LIBRARY}" DIRECTORY)
                list(APPEND ALPAKA_VENDOR_RPATH "${_alpaka_rccl_dir}")
            endif()
        else()
            message(WARNING "RCCL library found but headers missing; skipping RCCL enablement")
        endif()
    else()
        message(STATUS "RCCL not found -> collective providers will use fallback path")
    endif()
else()
    message(STATUS "RCCL support disabled or collectives disabled")
endif()
