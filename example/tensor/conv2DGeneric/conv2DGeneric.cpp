...
int runConv2DBasic(Cfg const& cfg){
    ...
    try {
        ...
        std::cout << "Weight: [" << out_channels << "x" << in_channels << "x" << kernel_h << "x" << kernel_w << "]" << std::endl;
    ...
    tensor::Tensor4D<float, decltype(device)> weight(device,{out_channels, in_channels, kernel_h, kernel_w});
        ...
        auto output_shape = tensor::ops::compute_conv2d_output_shape(
            input.shape(), weight.shape(), params);
        std::cout << "Expected output: [" << output_shape[0] << "x" << output_shape[1] 
                  << "x" << output_shape[2] << "x" << output_shape[3] << "]" << std::endl;
        ...
        for (std::size_t oc = 0; oc < out_channels; ++oc) {
            for (std::size_t ic = 0; ic < in_channels; ++ic) {
                if (kernel_h >= 3 && kernel_w >= 3) {
                    std::size_t kernel_offset = oc * (in_channels * kernel_h * kernel_w) + 
    ...
    auto output = tensor::ops::conv2d<float>(exec, device, queue, input, weight, params);
        ...
        std::size_t total_ops = output_shape[0] * output_shape[1] * output_shape[2] * output_shape[3] *
                               in_channels * kernel_h * kernel_w * 2; // multiply-add = 2 ops
