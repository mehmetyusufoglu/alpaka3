...
int runHighLevelConv2D(Cfg const& cfg){
    ...
    if(std::getenv("ALPAKA_CONV2D_FORCE_TILED") == nullptr){
        ...
        std::cout << "[example] ALPAKA_CONV2D_FORCE_TILED already set to "
    ...
    try {
        ...
        std::cout << "Weight: [" << out_channels << ", " << in_channels << ", " << kernel_size << ", " << kernel_size << "]" << std::endl;
    ...
    tensor::Tensor4D<float, decltype(device)> weight(device, {out_channels, in_channels, kernel_size, kernel_size});
