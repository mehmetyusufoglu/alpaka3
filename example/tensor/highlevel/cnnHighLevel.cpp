...
void fillRandom(TTensor &t){
    ...
    auto* p = t.hostData();
    for(std::size_t i=0;i<t.size();++i) p[i]=dist(gen);
    t.markHostModified();
}
...
int runCnn(Cfg const& cfg){
    ...
    struct Stat { std::string name; double ms; }; std::vector<Stat> stats; auto t0 = stamp();
    ...
    alpaka::tensor::Tensor1D<float, decltype(device)> bnScale(device,{8},"bnScale"); fillRandom(bnScale);
    alpaka::tensor::Tensor1D<float, decltype(device)> bnBias(device,{8},"bnBias"); fillRandom(bnBias);
    alpaka::tensor::Tensor1D<float, decltype(device)> bnMean(device,{8},"bnMean"); fillRandom(bnMean);
    alpaka::tensor::Tensor1D<float, decltype(device)> bnVar(device,{8},"bnVar");
    {
        auto* v = bnVar.hostData(); for(size_t i=0;i<8;++i) v[i] = 1.0f + 0.5f*i; bnVar.markHostModified();
    }

    alpaka::tensor::Tensor4D<float, decltype(device)> bnOut(device,{1,8,16,16},"bnOut");
    auto t_bn_s = stamp();
    alpaka::tensor::ops::batch_norm_inference<float>(exec, device, queue, convOut, bnScale, bnBias, bnMean, bnVar, 1e-5f, bnOut);
    ...
    auto t_pool_s = stamp();
    auto poolOut = alpaka::tensor::ops::max_pool2d<float>(exec, device, queue, bnOut, poolP); // [1,8,8,8] (bnOut now holds ReLU output)
    ...
    alpaka::tensor::Tensor1D<float, decltype(device)> W(device,{512*10},"W"); fillRandom(W);
    alpaka::tensor::Tensor1D<float, decltype(device)> bias(device,{10},"bias"); fillRandom(bias);
    alpaka::tensor::Tensor1D<float, decltype(device)> logits(device,{10},"logits"); logits.fill(0.f);
    ...
    {
        auto* src = logits.hostData(); auto* dst = logits2D.hostData();
        for(size_t i=0;i<10;++i) dst[i]=src[i];
    ...
    auto* p = probs.hostData(); std::cout << "Probabilities: ";
    for(size_t i=0;i<10;++i) std::cout << p[i] << ' ';
    std::cout << "\n";

    // Argmax
    size_t best=0; for(size_t i=1;i<10;++i) if(p[i]>p[best]) best=i;
    std::cout << "Predicted class: " << best << " (p=" << p[best] << ")\n";
    auto total_ms = ms(t0, stamp());
    std::cout << "--- Timing (ms) ---\n";
    for(auto const& s : stats) std::cout << s.name << ": " << s.ms << " ms\n";
