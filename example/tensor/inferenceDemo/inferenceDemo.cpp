...
int runInference(Tag const& tag)
{
    ...
    {
        auto* p = conv1W.hostData();
        for(std::size_t i = 0; i < conv1W.size(); ++i)
            p[i] = 0.05f;
        conv1W.markHostModified();
    }
    ...
    pipe.add(layers::maxPool2d<Device>(ops::Pool2DParams{2, 2, 2, 2, 0, 0}));
    pipe.add(layers::flatten<Device>());
    pipe.add(layers::linear<Device>(N, FC_OUT));
    pipe.add(layers::softmax<Device>(N, FC_OUT));

    // Run pipeline
    typename Pipe::Any inputAny = std::move(input);
    auto resultAny = pipe.forward(std::move(inputAny));
