...
namespace
{
    ...
    int runFft3d(Tag const& tag, Fft3dOptions const& options)
    {
        ...
        bool const providerAvailable = tt::EnabledVendorLibs::hasCUFFT && context.supportsOperation(tt::OpType::FFT);
        ...
        if(options.verbose)
        {
            auto const* hostSpectrum = deviceOutput.hostData();
            std::cout << "First 6 frequency bins (complex values):\n";
            std::size_t const preview = std::min<std::size_t>(6, totalElements);
            for(std::size_t idx = 0; idx < preview; ++idx)
            {
                std::cout << "  bin[" << idx << "] = " << hostSpectrum[idx] << '\n';
            }
        }
