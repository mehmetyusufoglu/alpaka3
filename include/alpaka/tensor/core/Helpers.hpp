...
namespace alpaka::tensor::helpers
{
    ...
    template<typename D>
    using F32T1 = Tensor1D<float, D>;
    template<typename D>
    using F32T2 = Tensor2D<float, D>;
    template<typename D>
    using F32T3 = Tensor3D<float, D>;
    template<typename D>
    using F32T4 = Tensor4D<float, D>;

    template<typename D>
    using F64T1 = Tensor1D<double, D>;
    ...
    template<typename T, typename Device>
    auto makeTensor3D(Device const& dev, std::size_t d0, std::size_t d1, std::size_t d2, std::string name = "")
        -> Tensor<T, 3, Device>
    {
    ...
    template<typename T, std::size_t Rank, typename Device>
    T* writableHostData(Tensor<T, Rank, Device>& t)
    {
        return t.hostData();
    }

    template<typename T, std::size_t Rank, typename Device>
    T const* readableHostData(Tensor<T, Rank, Device> const& t)
    {
        return t.hostData();
    }
