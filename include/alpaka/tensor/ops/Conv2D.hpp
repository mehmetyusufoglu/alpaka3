...
namespace alpaka::tensor::ops {
    ...
    struct Conv2DParams {
        ...
        Conv2DParams(std::size_t sh, std::size_t sw, std::size_t ph, std::size_t pw, 
                     std::size_t dh = 1, std::size_t dw = 1)
            : stride_h(sh), stride_w(sw), pad_h(ph), pad_w(pw), dilation_h(dh), dilation_w(dw) {}
    };
    ...
    public:
        ...
        ALPAKA_FN_ACC void operator()(
            ...
            std::size_t N, std::size_t C_in, std::size_t C_out,
            ...
            Conv2DParams params) const
        {
            ...
            for(auto [n, c_out, h_out, w_out] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid, 
                ...
                for(std::size_t c_in = 0; c_in < C_in; ++c_in) {
                    for(std::size_t kh = 0; kh < K_h; ++kh) {
                        for(std::size_t kw = 0; kw < K_w; ++kw) {
                            ...
                            int h_in = static_cast<int>(h_out * params.stride_h + kh * params.dilation_h) - static_cast<int>(params.pad_h);
                            int w_in = static_cast<int>(w_out * params.stride_w + kw * params.dilation_w) - static_cast<int>(params.pad_w);
                            
                            // Check bounds (padding) and tensor extents
                                     if(h_in >= 0 && h_in < static_cast<int>(H_in) && 
                                         w_in >= 0 && w_in < static_cast<int>(W_in)) {
                                
                                alpaka::Vec<std::size_t, 4> input_idx{n, c_in, static_cast<std::size_t>(h_in), static_cast<std::size_t>(w_in)};
    ...
    public:
            ...
            alpaka::concepts::CVector auto sharedMemExtents) const
        {
            ...
            for(auto [n, c_out] : onAcc::makeIdxMap(acc, onAcc::worker::threadsInGrid, 
                    IdxRange{Vec{N, C_out}})) {
                
                // Spatial tiling following StencilKernel pattern
                for(auto blockStartIdx : onAcc::makeIdxMap(acc, onAcc::worker::blocksInGrid,
                    ...
                    for(std::size_t c_in = 0; c_in < C_in; ++c_in) {
                        ...
                        for(auto idx2d : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInBlock, 
                                IdxRange{patchSize})) {
                            
                            // Convert patch coordinates to input coordinates
                            int h_in = static_cast<int>(blockStartIdx[0] * params.stride_h) - static_cast<int>(params.pad_h) + static_cast<int>(idx2d[0]);
                            int w_in = static_cast<int>(blockStartIdx[1] * params.stride_w) - static_cast<int>(params.pad_w) + static_cast<int>(idx2d[1]);
                            
                            // Bounds checking with extra safety
                            if(h_in >= 0 && h_in < static_cast<int>(H_in) && 
                               w_in >= 0 && w_in < static_cast<int>(W_in) &&
                               n < N && c_in < C_in) {
                                Vec<std::size_t, 4> input_idx{n, c_in, static_cast<std::size_t>(h_in), static_cast<std::size_t>(w_in)};
                        ...
                        for(auto outIdx : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInBlock,
                                IdxRange{chunkSize})) {
                            
                            auto globalOutIdx = outIdx + blockStartIdx;
                            if(globalOutIdx[0] >= H_out || globalOutIdx[1] >= W_out) continue;
                            ...
                            // Accumulate to output with bounds checking
                                     if(n < N && c_out < C_out && 
                                         globalOutIdx[0] < H_out && globalOutIdx[1] < W_out) {
    ...
    public:
        ...
        for(auto tileOrigin : alpaka::onAcc::makeIdxMap(
            ...
            alpaka::IdxRange{alpaka::Vec{std::size_t{0}, std::size_t{0}, std::size_t{0}, std::size_t{0}},
                     alpaka::Vec{N, C_out, H_out, W_out},
                     alpaka::Vec{std::size_t{1}, std::size_t{1}, std::size_t{TILE_H}, std::size_t{TILE_W}}})) {
        const std::size_t n = tileOrigin[0];
        const std::size_t c_out = tileOrigin[1];
        const std::size_t outH0 = tileOrigin[2];
        const std::size_t outW0 = tileOrigin[3];
                ...
                for(auto tIdx : alpaka::onAcc::makeIdxMap(
                        ...
                        alpaka::IdxRange{alpaka::Vec{std::size_t{1}, std::size_t{1}, std::size_t{TILE_H}, std::size_t{TILE_W}}})) {
                ...
                const std::size_t loadH = K_h + TILE_H - 1;
                const std::size_t loadW = K_w + TILE_W - 1;
                const std::size_t tileEffH = (outH0 + TILE_H <= H_out) ? TILE_H : (H_out - outH0);
                const std::size_t tileEffW = (outW0 + TILE_W <= W_out) ? TILE_W : (W_out - outW0);
                ...
                for(std::size_t c_in = 0; c_in < C_in; ++c_in) {
                    ...
                    for(auto tIdx : alpaka::onAcc::makeIdxMap(
                        ...
                        for(std::size_t ph = tIdx[2]; ph < loadH; ph += TILE_H) {
                            for(std::size_t pw = tIdx[3]; pw < loadW; pw += TILE_W) {
                                int h_in = static_cast<int>(outH0) - static_cast<int>(params.pad_h) + static_cast<int>(ph);
                                int w_in = static_cast<int>(outW0) - static_cast<int>(params.pad_w) + static_cast<int>(pw);
                                float val = 0.f;
                                if(h_in >= 0 && h_in < static_cast<int>(H_in) && w_in >= 0 && w_in < static_cast<int>(W_in)) {
                                    Vec<std::size_t, 4> inIdx{n, c_in, static_cast<std::size_t>(h_in), static_cast<std::size_t>(w_in)};
                                    val = input[inIdx];
                                }
                    ...
                    for(auto tIdx : alpaka::onAcc::makeIdxMap(
                        ...
                        const std::size_t localH = tIdx[2];
                        const std::size_t localW = tIdx[3];
                        float partial = 0.f;
                        for(std::size_t kh = 0; kh < K_h; ++kh) {
                            for(std::size_t kw = 0; kw < K_w; ++kw) {
                        ...
                        if(c_in == 0) output[oIdx] = partial; else output[oIdx] += partial;
                    }
                    alpaka::onAcc::syncBlockThreads(acc);
                } // c_in
            } // tiles
    ...
    namespace detail {
        ...
        template<typename T>
        void cudnn_conv2d_impl(const T*, const T*, T*, 
                               std::size_t, std::size_t, std::size_t, 
                               std::size_t, std::size_t, std::size_t, std::size_t, 
                               std::size_t, std::size_t, const Conv2DParams&) {
    ...
    auto compute_conv2d_output_shape(
        const Shape& input_shape,   // [N, C_in, H_in, W_in]
        const Shape& weight_shape,  // [C_out, C_in, K_h, K_w]
        const Conv2DParams& params) -> std::array<std::size_t, 4>
    {
    ...
    assert(output_shape[2] > 0 && output_shape[3] > 0 && "Conv2D: computed output spatial size must be > 0 (check stride/pad/dilation)");
    ...
    assert(input_shape[2] + 2*params.pad_h >= params.dilation_h * (weight_shape[2]-1) + 1 && "Conv2D: padding/dilation produce negative effective H_out");
    assert(input_shape[3] + 2*params.pad_w >= params.dilation_w * (weight_shape[3]-1) + 1 && "Conv2D: padding/dilation produce negative effective W_out");
    ...
    output.ensureOnDevice(device, queue);
        ...
        std::cout << "Conv2D: Processing 4D convolution with output shape [" << N << "," << C_out << "," << H_out << "," << W_out << "]" << std::endl;
        
        // Use alpaka kernel for all backends including GPU
        std::cout << "Conv2D: Using alpaka 4D kernel for backend acceleration" << std::endl;
        
        // Heuristic: attempt tiled Option B path when conditions satisfied (stride/dilation == 1 and kernel size within template bounds)
        // Compile-time configuration defaults (can be hoisted to a config header later)
        constexpr int TILE_H = 16;
        constexpr int TILE_W = 16;
        constexpr int MAX_KH = 7; // supports runtime K_h up to 7
        constexpr int MAX_KW = 7; // supports runtime K_w up to 7

        bool canTile = (params.stride_h == 1 && params.stride_w == 1 &&
                        params.dilation_h == 1 && params.dilation_w == 1 &&
                        K_h <= static_cast<std::size_t>(MAX_KH) &&
                        K_w <= static_cast<std::size_t>(MAX_KW));
        ...
        if(const char* forceEnv = std::getenv("ALPAKA_CONV2D_FORCE_TILED")){
            if(forceEnv[0] == '1') {
        ...
        if(canTile) {
            ...
            // Frame spec: one frame (all work in single kernel invocation); threads selection managed by alpaka backend config.
            // Thread block extent = tile dims; blocksInGrid now spans 4D (N,C_out,H_out,W_out) with chunk (1,1,TILE_H,TILE_W)
            // 4D thread space (1,1,TILE_H,TILE_W) to align with 4D blocksInGrid selection
            auto frameExtent = alpaka::Vec{std::size_t{1}, std::size_t{1}, std::size_t{TILE_H}, std::size_t{TILE_W}};
            auto numFrames = alpaka::Vec{std::size_t{1}, std::size_t{1}, std::size_t{1}, std::size_t{1}}; // single frame
            auto frameSpec = alpaka::onHost::FrameSpec{numFrames, frameExtent};

            queue.enqueue(exec, frameSpec, Conv2DTiledKernelOptionB<TILE_H, TILE_W, MAX_KH, MAX_KW>{},
                ...
                N, C_in, C_out, H_in, W_in, H_out, W_out, K_h, K_w, params);
        } else {
            ...
            auto frameExtent = alpaka::Vec{std::size_t{1}, std::size_t{1}, std::size_t{1}, W_out};  // Process one row with W_out columns
            auto numFrames = alpaka::Vec{N, C_out, H_out, std::size_t{1}};    // N * C_out * H_out frames total
            
            auto frameSpec = alpaka::onHost::FrameSpec{numFrames, frameExtent};
            
            std::cout << "Conv2D: Using frame spec with numFrames=[" << numFrames[0] << "," << numFrames[1] << "," << numFrames[2] << "," << numFrames[3] 
                      << "] frameExtent=[" << frameExtent[0] << "," << frameExtent[1] << "," << frameExtent[2] << "," << frameExtent[3] << "]" << std::endl;
            
            queue.enqueue(exec, frameSpec, Conv2DKernel{},
                const_cast<tensor::Tensor4D<T,Device>&>(input).deviceBuffer(device, queue),
                const_cast<tensor::Tensor4D<T,Device>&>(weight).deviceBuffer(device, queue),
                output.deviceBuffer(device, queue),
                N, C_in, C_out, H_in, W_in, H_out, W_out, K_h, K_w, params);
