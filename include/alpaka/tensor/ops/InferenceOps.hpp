    ...
    Pool2DParams const& p)
{
    ...
    assert(p.kernel_h>0 && p.kernel_w>0 && p.stride_h>0 && p.stride_w>0 && "Pool2D: kernel/stride must be >0");
...
namespace detail {
    ...
    struct BiasAdd2DKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, BiasBuf b, OutBuf out,
                                      std::size_t M, std::size_t N, std::size_t total) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                                       alpaka::IdxRange{total})) {
                auto row = idx / N; auto col = idx % N;
    ...
    struct BiasAdd4DKernel {
        template<typename Acc, typename InBuf, typename BiasBuf, typename OutBuf>
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, BiasBuf b, OutBuf out,
                                      std::size_t N, std::size_t C, std::size_t H, std::size_t W, std::size_t total) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                                       alpaka::IdxRange{total})) {
                auto hw = H*W; auto cStride = hw; auto nStride = C*hw;
                auto n = idx / nStride;
    ...
    struct LinearBiasKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, OutBuf out, BiasBuf b,
                                      std::size_t M, std::size_t N, std::size_t total) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                                       alpaka::IdxRange{total})) {
                auto col = idx % N; out[idx] += b[col];
            }
        }
    };
    ...
    struct Softmax2DKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, OutBuf out,
    ...
    struct MaxPool2DKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, OutBuf out,
            ...
            std::size_t H_out, std::size_t W_out, Pool2DParams p) const {
            for(auto [n,c,h_out,w_out] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                ...
                h_start = std::max(h_start, 0); w_start = std::max(w_start, 0);
                T mx = std::numeric_limits<T>::lowest();
                for(int ih=h_start; ih<h_end; ++ih)
                    for(int iw=w_start; iw<w_end; ++iw)
                        mx = alpaka::math::max(mx, in[alpaka::Vec<std::size_t,4>{n,c,(std::size_t)ih,(std::size_t)iw}]);
    ...
    struct AvgPool2DKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, OutBuf out,
            ...
            for(auto [n,c,h_out,w_out] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                ...
                T sum = 0; int count=0;
                for(int ih=h_start; ih<h_end; ++ih)
                    for(int iw=w_start; iw<w_end; ++iw){ sum += in[alpaka::Vec<std::size_t,4>{n,c,(std::size_t)ih,(std::size_t)iw}]; ++count; }
    ...
    struct BatchNormInferenceKernel {
        ...
        template<typename Acc, typename InBuf, typename ScaleBuf, typename BiasBuf, typename MeanBuf, typename InvStdBuf, typename OutBuf>
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, ScaleBuf gamma, BiasBuf beta, MeanBuf mean, InvStdBuf invStd, OutBuf out,
            std::size_t N, std::size_t C, std::size_t H, std::size_t W, std::size_t total) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                                       alpaka::IdxRange{total})) {
                auto hw = H*W; auto nStride=C*hw;
                auto n = idx / nStride;
                auto rem = idx % nStride; auto c = rem / hw; auto rem2 = rem % hw; auto h = rem2 / W; auto w = rem2 % W;
                auto coord = alpaka::Vec<std::size_t,4>{n,c,h,w};
                T x = in[coord];
    ...
    struct ConcatChannelsKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, ABuf A, BBuf B, OutBuf O,
            std::size_t N, std::size_t C1, std::size_t C2, std::size_t H, std::size_t W, std::size_t total) const {
            auto C_tot = C1 + C2; auto hw = H*W; auto nStride = C_tot*hw; (void)N;
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                                       alpaka::IdxRange{total})) {
                auto n = idx / nStride;
                auto rem = idx % nStride; auto c = rem / hw; auto rem2 = rem % hw; auto h = rem2 / W; auto w = rem2 % W;
...
void bias_add_2d(Exec const& exec, Device const& device, Queue& queue,
                 ...
                 tensor::Tensor2D<T, Device>& output)
{
    ...
    queue.enqueue(exec, frame, detail::BiasAdd2DKernel<T>{}, input.deviceBuffer(device, queue), bias.deviceBuffer(device, queue), output.deviceBuffer(device, queue), M, N, total);
...
template<typename T, typename Exec, typename Device, typename Queue>
void bias_add_4d(Exec const& exec, Device const& device, Queue& queue,
                 ...
                 tensor::Tensor4D<T, Device>& output)
{
    ...
    auto frame = ops::detail::makeFrame<Exec, Queue>(total);
    queue.enqueue(exec, frame, detail::BiasAdd4DKernel<T>{}, input.deviceBuffer(device, queue), bias.deviceBuffer(device, queue), output.deviceBuffer(device, queue), N, C, H, W, total);
    ::alpaka::onHost::wait(queue);
    output.markDeviceModified(device, queue);
}
...
void linear(Exec const& exec, Device const& device, Queue& queue,
            ...
            tensor::Tensor1D<float, Device>& Out)    // flattened M*N
{
    ...
    if(bias) {
        ...
        std::size_t total = M * N;
        auto frame = ops::detail::makeFrame<Exec, Queue>(total);
    queue.enqueue(exec, frame, detail::LinearBiasKernel{}, Out.deviceBuffer(device, queue), bias->deviceBuffer(device, queue), M, N, total);
...
template<typename T, typename Exec, typename Device, typename Queue>
void softmax_2d(Exec const& exec, Device const& device, Queue& queue,
                tensor::Tensor2D<T, Device>& input,
                tensor::Tensor2D<T, Device>& output)
{
    auto inShape = input.shape();
    ...
    queue.enqueue(exec, frame, detail::Softmax2DKernel<T>{}, input.deviceBuffer(device, queue), output.deviceBuffer(device, queue), M, N);
    output.markDeviceModified(device, queue); // defer wait
}
...
namespace detail {
    ...
    struct FlattenCopyKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, T const* in, T* out, std::size_t total) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid, alpaka::IdxRange{total})) {
                out[idx] = in[idx];
            }
        }
    };
    ...
    struct Copy1DTo2DKernel {
        ...
        ALPAKA_FN_ACC void operator()(Acc const& acc, InBuf in, OutBuf out, std::size_t M, std::size_t N) const {
            for(auto [idx] : alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid, alpaka::IdxRange{M*N})) {
                auto row = idx / N; auto col = idx % N;
...
inline bool isContiguous(auto const&) { return true; }

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue>
tensor::Tensor1D<T, Device> flatten(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& src) {
    ...
    if(isContiguous(src)) {
        ...
        // Reuse underlying storage by moving then restoring (minimal intrusive change): create new tensor and manually assign buffers.
    ...
    src.ensureOnDevice(device, queue); out.ensureOnDevice(device, queue);
    auto total = src.size();
    auto frame = ops::detail::makeFrame<Exec, Queue>(total);
    queue.enqueue(exec, frame, detail::FlattenCopyKernel<T>{}, src.deviceBuffer(device, queue).data(), out.deviceBuffer(device, queue).data(), total);
...
template<typename T, typename Exec, typename Device, typename Queue>
tensor::Tensor2D<T,Device> copy_flat_to_2d(Exec const& exec, Device const& device, Queue& queue,
    ...
    flat.ensureOnDevice(device, queue); out.ensureOnDevice(device, queue);
    auto frame = ops::detail::makeFrame<Exec, Queue>(M*N);
    queue.enqueue(exec, frame, detail::Copy1DTo2DKernel<T>{}, flat.deviceBuffer(device, queue).data(), out.deviceBuffer(device, queue), M, N);
    out.markDeviceModified(device, queue);
    return out;
}
    ...
    Pool2DParams const& params)
{
    ...
    input.ensureOnDevice(device, queue); output.ensureOnDevice(device, queue);
    auto N=inShape[0]; auto C=inShape[1]; auto H=inShape[2]; auto W=inShape[3];
    auto H_out=outShape[2]; auto W_out=outShape[3];
    ...
    queue.enqueue(exec, frameSpec, detail::MaxPool2DKernel<T>{}, input.deviceBuffer(device, queue), output.deviceBuffer(device, queue),
                  N,C,H,W,H_out,W_out, params);
    output.markDeviceModified(device, queue); // defer wait
    return output;
}
    ...
    Pool2DParams const& params)
{
    ...
    auto numFrames = alpaka::Vec{N,C,H_out,std::size_t{1}};
    auto frameSpec = alpaka::onHost::FrameSpec{numFrames, frameExtent};
    queue.enqueue(exec, frameSpec, detail::AvgPool2DKernel<T>{}, input.deviceBuffer(device, queue), output.deviceBuffer(device, queue),
                  N,C,H,W,H_out,W_out, params);
    output.markDeviceModified(device, queue); // defer wait
    return output;
}
    ...
    tensor::Tensor4D<T,Device>& output)
{
    ...
    auto C = inShape[1];
    assert(scale.shape()[0]==C && bias.shape()[0]==C && running_mean.shape()[0]==C && running_var.shape()[0]==C && "BatchNorm: parameter sizes must match channels");
    ...
    tensor::Tensor1D<T,Device> invStd(device, {C}, "bn_invstd");
    {
        ...
        for(std::size_t c=0;c<C;++c) invStdHost[c] = T{1} / static_cast<T>(std::sqrt(varHost[c] + epsilon));
        invStd.markHostModified();
    }

    input.ensureOnDevice(device, queue); output.ensureOnDevice(device, queue);
    scale.ensureOnDevice(device, queue); bias.ensureOnDevice(device, queue);
    running_mean.ensureOnDevice(device, queue); invStd.ensureOnDevice(device, queue);
    auto N=inShape[0]; auto H=inShape[2]; auto W=inShape[3];
    std::size_t total = N*C*H*W;
    auto frame = ops::detail::makeFrame<Exec, Queue>(total);
    queue.enqueue(exec, frame, detail::BatchNormInferenceKernel<T>{},
...
{
    auto sa=a.shape(); auto sb=b.shape();
    ...
    a.ensureOnDevice(device, queue); b.ensureOnDevice(device, queue); out.ensureOnDevice(device, queue);
    auto N=sa[0]; auto C1=sa[1]; auto C2=sb[1]; auto H=sa[2]; auto W=sa[3];
    std::size_t total = N*(C1+C2)*H*W;
    auto frame = ops::detail::makeFrame<Exec, Queue>(total);
    queue.enqueue(exec, frame, detail::ConcatChannelsKernel<T>{}, a.deviceBuffer(device, queue), b.deviceBuffer(device, queue), out.deviceBuffer(device, queue), N, C1,C2, H,W, total);
    out.markDeviceModified(device, queue); // defer wait
    return out;
}

} // namespace alpaka::tensor::ops (additional)
