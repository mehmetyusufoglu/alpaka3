...
#include <alpaka/tensor/TensorSimple.hpp>
    ...
    namespace tensor
    {
        namespace ops
        {
            ...
            Tensor<T, Rank> add(const Tensor<T, Rank>& a, 
                ...
                Tensor<T, Rank> result(a.shape(), a.device(), a.dtype(), a.layout(), "add_result");
            ...
            Tensor<T, Rank> relu(const Tensor<T, Rank>& input) {
                ...
                // Create output tensor
                Tensor<T, Rank> result(input.shape(), input.device(), input.dtype(), input.layout(), "relu_result");
