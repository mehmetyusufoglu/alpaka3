...
#include <alpaka/tensor/ops/Layer.hpp> // for Conv2DLayerStruct convenience maker
    ...
    bool& alreadyFolded)
{
    ...
    assert(gamma.shape()[0]==C_out && beta.shape()[0]==C_out && running_mean.shape()[0]==C_out && running_var.shape()[0]==C_out && "BN fold: parameter size mismatch");
    weights.toHost(device, queue);
    gamma.toHost(device, queue); beta.toHost(device, queue);
    running_mean.toHost(device, queue); running_var.toHost(device, queue);
    if(!bias) bias.emplace(device, std::array<std::size_t,1>{C_out}, "conv_bias");
    ...
    for(std::size_t c=0;c<C_out;++c){
        ...
        for(std::size_t i=0;i<perChannel;++i) wChan[i] *= scale;
        float bOrig = bHost[c];
        bHost[c] = beHost[c] + scale * (bOrig - mHost[c]);
    }
    weights.markHostModified(); bias->markHostModified();
    weights.ensureOnDevice(device, queue); bias->ensureOnDevice(device, queue);
    alreadyFolded = true;
...
template<typename Exec, typename Device, typename Queue>
std::pair< tensor::Tensor4D<float,Device>, tensor::Tensor1D<float,Device> >
clone_and_fold_batch_norm(
    Exec const& exec,
    Device& device,
    Queue& queue,
    ...
    float epsilon)
{
    ...
    if(biasSrc){
        auto* dst = bias.hostData(); auto* src = biasSrc->hostData();
        for(std::size_t i=0;i<weights.shape()[0];++i) dst[i]=src[i];
        bias.markHostModified();
    } else {
        bias.fill(0.f);
    }
    bool folded=false;
    auto gammaCopy = gamma; auto betaCopy=beta; auto meanCopy=running_mean; auto varCopy=running_var; // copies so fold function signature satisfied
    std::optional< tensor::Tensor1D<float,Device> > biasOpt = bias; // copy into optional
    fold_batch_norm_into_conv(exec, device, queue, weights, biasOpt, gammaCopy, betaCopy, meanCopy, varCopy, epsilon, folded);
...
Conv2DLayerStruct<Device> make_conv2d_layer_with_bn_folding(
    ...
    Conv2DParams params = {})
{
    auto [wFold, bFold] = clone_and_fold_batch_norm(exec, device, queue, weightsSrc, biasSrc, gamma, beta, running_mean, running_var, epsilon);
