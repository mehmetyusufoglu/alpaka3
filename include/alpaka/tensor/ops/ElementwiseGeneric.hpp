...
tensor::Tensor<T, Rank, Device> binary(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& a, tensor::Tensor<T, Rank, Device>& b, Functor f, const char* name="binary") {
    ...
    queue.enqueue(exec, frame, BinaryKernel{}, a.deviceBuffer(device, queue), b.deviceBuffer(device, queue), out.deviceBuffer(device, queue), n, f);
...
// out = f(in)
template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue, typename Functor>
tensor::Tensor<T, Rank, Device> unary(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& in, Functor f, const char* name="unary") {
    in.ensureOnDevice(device, queue);
    tensor::Tensor<T, Rank, Device> out(device, in.shape(), name);
    out.ensureOnDevice(device, queue);
    auto n = in.size();
    auto frame = detail::makeFrame<Exec, Queue>(n);
    queue.enqueue(exec, frame, UnaryKernel{}, in.deviceBuffer(device, queue), out.deviceBuffer(device, queue), n, f);
    ::alpaka::onHost::wait(queue);
    out.markDeviceModified(device, queue);
    out.toHost(device, queue);
    return out;
...
tensor::Tensor<T, Rank, Device> add(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& a, tensor::Tensor<T, Rank, Device>& b) {
    return binary<T, Rank>(exec, device, queue, a, b, AddOp{}, "add");
}

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue>
tensor::Tensor<T, Rank, Device> sub(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& a, tensor::Tensor<T, Rank, Device>& b) {
    return binary<T, Rank>(exec, device, queue, a, b, SubOp{}, "sub");
}

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue>
tensor::Tensor<T, Rank, Device> mul(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& a, tensor::Tensor<T, Rank, Device>& b) {
    return binary<T, Rank>(exec, device, queue, a, b, MulOp{}, "mul");
}

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue>
tensor::Tensor<T, Rank, Device> div(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& a, tensor::Tensor<T, Rank, Device>& b) {
    return binary<T, Rank>(exec, device, queue, a, b, DivOp{}, "div");
}

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue>
tensor::Tensor<T, Rank, Device> relu(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& in) {
...
tensor::Tensor<T, Rank, Device> add_scalar(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& in, S scalar, const char* name="add_scalar") {
    return unary<T, Rank>(exec, device, queue, in, AddScalarOp<S>{scalar}, name);
}

template<typename T, std::size_t Rank, typename Exec, typename Device, typename Queue, typename S>
tensor::Tensor<T, Rank, Device> mul_scalar(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& in, S scalar, const char* name="mul_scalar") {
...
void relu_inplace(Exec const& exec, Device const& device, Queue& queue, tensor::Tensor<T, Rank, Device>& t) {
    ...
    auto n = t.size();
    auto frame = detail::makeFrame<Exec, Queue>(n);
    queue.enqueue(exec, frame, UnaryKernel{}, t.deviceBuffer(device, queue), t.deviceBuffer(device, queue), n, ReluOp{});
    ::alpaka::onHost::wait(queue);
